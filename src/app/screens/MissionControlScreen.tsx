...
async function fetchMissionControlMatches(dateStr: string, leagueId: string) {
  let query = supabase
    .from("matches")
    .select(`
      id, home_team, away_team, start_time, status, league_id, venue, referee,
      leagues (name, logo_url, country),
...
export function MissionControlScreen() {
  ...
  const mcQueryKey = queryKeys.missionControl.matches(dateStr, selectedLeagueId);

  const { data: matches = [], isLoading } = useQuery({
    queryKey: mcQueryKey,
    queryFn: () => fetchMissionControlMatches(dateStr, selectedLeagueId),
  });

  // Build tips map from matches data
  const tips: Record<string, any> = {};
  matches.forEach((m: any) => {
    if (m.tips) {
      const tip = Array.isArray(m.tips) ? m.tips[0] : m.tips;
      if (tip) tips[m.id] = tip;
    }
  });
  ...
  // Realtime subscription — updates TanStack Query cache directly
  useEffect(() => {
    const channel = supabase
      ...
      .on(
        ...
        (payload: any) => {
          queryClient.setQueryData(mcQueryKey, (old: any[] | undefined) => {
            if (!old) return old;
  ...
  }, [dateStr, selectedLeagueId, queryClient, mcQueryKey]);

  const handleSyncMatches = async () => {
    try {
      const { ingestMatches } = await import("@/lib/backend");
      await ingestMatches(dateStr);
      queryClient.invalidateQueries({ queryKey: mcQueryKey });
    } catch(e) {
      ...
      <div className="p-4 bg-white border-b sticky top-[60px] z-10">
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="px-4 flex gap-2 items-end">
                      ...
                      <PopoverTrigger asChild>
                        ...
                        >
                          {selectedLeagueId === "all"
                            ? "All Leagues"
                            : leagues.find((l: any) => String(l.id) === selectedLeagueId)?.name || "Select League..."}
                ...
                <Button variant="outline" onClick={handleSyncMatches} disabled={isLoading}>
                    Sync
                </Button>
            </div>
        </div>
